...
function Admin(storage) {
    XModule.call(this);
    this.logger = require('../../../core/Logger')('xep-0045:admin');
...
Admin.prototype.list = function (room, stanza, affiliation) {
    ...
    switch (affiliation) {
        case NS.MUC_AFFILIATION_ADMIN:
            affiliationSet.push(this.storage.RoomMembers.Affiliation.Admin);
            break;
        case NS.MUC_AFFILIATION_MEMBER:
            affiliationSet.push(this.storage.RoomMembers.Affiliation.Member);
            break;
        case NS.MUC_AFFILIATION_OWNER:
            affiliationSet.push(this.storage.RoomMembers.Affiliation.Owner);
            break;
        case NS.MUC_AFFILIATION_OUTCAST:
            affiliationSet.push(this.storage.RoomMembers.Affiliation.Outcast);
            break;
    }
    ...
    room.getMembers({
        ...
        self.logger.debug('members with affiliation ' + affiliation + ': ' + JSON.stringify(members));
...
Admin.prototype.handleAdminRequests = function (stanza) {
    this.logger.debug('handleAdminRequests');
    var self = this;
    ...
    if (item && (
        ...
        storage.findRoom(roomname).then(
            function (room) {
                self.logger.debug('found room: ' + JSON.stringify(room) + ' and ' + method);
