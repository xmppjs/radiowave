...
Presence.prototype.sendPresenceConfirmation = function (room, member, roomjid) {
  logger.debug('send presence confirmation from ' + roomjid + ' to ' + member.jid);
...
Presence.prototype.sendPresenceJoin = function (room, newMember, roomjid, nickname) {
  ...
  room.getMembers().then(function (members) {
    try {
      var newPresence = self.generatePresence(
...
Presence.prototype.handleOccupantPresence = function (stanza) {
  ...
  storage.findOrCreateUser(mucutil.getBareJid(userjid)).spread(function (u, created) { // jshint ignore:line
    user = u;

    // find room
    storage.findRoom(roomname).then(function (room) {
      // room exists
