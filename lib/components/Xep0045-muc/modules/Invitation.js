...
var util = require('util'),
  ...
  logger = require('../../../core/Logger')('xep-0045:invitation');
...
function Invitation(storage, options) {
  ...
  if (storage) {
    ...
    logger.warn('storage is not set properly');
  }
}
...
Invitation.prototype.match = function (stanza) {
  ...
  return (
    stanza.is('message') && 
    (
      stanza.getChild('x', NS.MUC_USER) && 
      stanza.getChild('x', NS.MUC_USER).getChild('invite')
    )
  ) ||
  (
    stanza.is('message') && 
    (
      stanza.getChild('x', NS.MUC_USER) && 
      stanza.getChild('x', NS.MUC_USER).getChild('decline')
    )
  );
};
...
Invitation.prototype.handleInvitations = function (stanza, x) {
  ...
  logger.debug(jid + ' invites ' + inviteejid + ' for ' + roomname);
  ...
  storage.sequelize.transaction().then(function (t) {
    transaction = t;
    return storage.findOrCreateUser(mucutil.getBareJid(inviteejid),{
      transaction: transaction
    })
  }).spread(function (member, created) { // jshint ignore:line
    invitee = member;
...
Invitation.prototype.handleDeclinedInvitations = function (stanza, x) {
  ...
  storage.sequelize.transaction().then(function (t) {
    ...
    return storage.declineMembership({
      room: room[0],
      invitee: invitee,
      invitor: invitor,
      reason: reasontxt,
