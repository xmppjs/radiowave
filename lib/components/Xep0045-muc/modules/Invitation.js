...
function Invitation(storage, options) {
  ...
  this.logger = require('../../../core/Logger')('xep-0045:invitation');
  ...
  if (storage) {
    ...
    this.logger.warn('storage is not set properly');
  }
}
...
Invitation.prototype.declineEventHandler = function (inviteRequest) {
  ...
  this.logger.debug('got decline for ' + roomjid.toString() + ' from ' + invitee + ' with reason: ' + reason);
...
Invitation.prototype.handleInvitations = function (stanza, x) {
  ...
  storage.findOrCreateUser(mucutil.getBareJid(inviteejid)).spread(function (member, created) {
    invitee = member;
...
Invitation.prototype.handleDeclinedInvitations = function (stanza, x) {
  this.logger.debug('handle declined invitations');
  var self = this;
