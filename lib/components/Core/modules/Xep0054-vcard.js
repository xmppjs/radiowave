...
function VCard() {
    XModule.call(this);
    this.logger = require('../../../core/Logger')('vcard');
    this.logger.info('load ' + this.name);
}
util.inherits(VCard, XModule);
...
VCard.prototype.match = function (stanza) {
    ...
    if (stanza.is('iq') && stanza.attrs.type === 'get' && (stanza.getChild('vcard', NS_VCARD))) {
        this.logger.debug('detected vcard get request');
        result = true;
    }

    // vCard Set Request
    if (stanza.is('iq') && stanza.attrs.type === 'set' && (stanza.getChild('vcard', NS_VCARD))) {
        this.logger.debug('detected vcard set request');
        result = true;
    }
