...
Owner.prototype.match = function (stanza) {
    // owner requests
    return (stanza.is('iq') && stanza.getChild('pubsub', NS.PUBSUB_OWNER) && stanza.getChild('pubsub', NS.PUBSUB_OWNER).getChild('create')) ||
    // configure requests
    (stanza.is('iq') && stanza.getChild('pubsub', NS.PUBSUB_CONFIG) && stanza.getChild('pubsub', NS.PUBSUB_OWNER).getChild('delete'));
};
...
Owner.prototype.handle = function (stanza) {
    ...
    this.storage.findUser(fromJid).then(function (user) {
        ...
        case 'delete':
            pubutil.findNode(self.storage, nodename, user, false).then(function (node) {
