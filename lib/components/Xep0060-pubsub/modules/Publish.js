...
var Publish = function (storage, options) {
  XModule.call(this);
  this.logger = require('../../../core/Logger')('xep-0060:publish');
...
Publish.prototype.extractItems = function (publishEl) {
  ...
  this.logger.debug('found items:  ' + childs.length);
  ...
  this.logger.debug('With Payload:  ' + itemswithpayload.toString());
  this.logger.debug('W/out Payload: ' + itemswithoutpayload.toString());
...
Publish.prototype.deliverPayload = function (node, publishEl) {
  ...
  return new Promise(function (resolve) {
    ...
    node.getConfigurations({
      ...
      self.logger.debug('preference of node: ' + JSON.stringify(conf));
      var deliver = true;
...
Publish.prototype.handlePublish = function (user, node, stanza, publish) {
  this.logger.debug('handlePublish');
  var self = this;
  this.logger.debug(stanza.toString());

  // if node is available
  if (node) {
    this.logger.debug('PUBLISH to ' + node.name);
    this.logger.debug(stanza.toString());
    ...
    this.deliverPayload(node).then(function (deliver) {
      self.logger.debug('deliver payload: ' + deliver);
      ...
      node.getSubscribers().then(
        function (subscribers) {
          self.logger.debug('send events to subs ' + JSON.stringify(subscribers));
    ...
    this.logger.error('node not there');
  }
};
