 ...
 */
Publish.prototype.isDeliveryWithPayload = function (node) {
...
Publish.prototype.handlePublish = function (stanza) {
  ...
  var node, user, msg = null;

  storage.findUser(fromJid).then(function (u) {
    user = u;
    ...
    return node.isSubscriber(user);
   }).then(function () {
    // check if we need to deliver the payload to subscriber
    return self.isDeliveryWithPayload(node);
