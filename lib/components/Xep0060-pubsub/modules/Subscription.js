...
Subscription.prototype.handleSubscribe = function (stanza, subcribeEl) {
  ...
  this.storage.findUser(fromJid.toString()).then(function (u) {
    user = u;
    return pubutil.findChannel(storage, nodename, user, self.options.autoCreateChannel)
  }).then(function (n) {
    node = n;
...
Subscription.prototype.handleUnsubscribe = function (stanza, unsubscribeEl) {
  ...
  this.storage.findUser(fromJid.toString()).then(function (u) {
    user = u;
    return pubutil.findChannel(storage, nodename, user, false)
  }).then(function (n) {
    node = n;
    ...
    return node.isSubscriber(user).catch(function(err){
      throw new Error('not-subscribed');
    })
  }).then(function () {
    // unregister subscriber
    logger.debug('user ' + fromJid.toString() + ' unsubscribes the node ' + node.name);
    return node.unsubscribe(user)
  }).then(function () {
