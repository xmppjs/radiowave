...
XRoute.prototype.listenTo = function (route) {
    ...
    route.on('stanza',function(stanza){
        if (stanza) {
            self.logger.debug(route.name + ' forward to ' + self.name +  ':  ' + stanza.attrs.from + ' -> ' + stanza.attrs.to);
            self.handle(stanza);
        }
    });
};
...
XRoute.prototype.chain = function (route) {
    ...
    route.on('send', function (stanza) {
        if (stanza) {
            self.logger.debug(route.name + ' send out to ' + self.name +  ':  ' + stanza.attrs.from + ' -> ' + stanza.attrs.to);
            self.send(stanza);
        }
    });
