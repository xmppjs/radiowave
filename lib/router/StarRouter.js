...
function StarRouter() {
    XRoute.call(this);
    this.logger = require('../core/Logger')('starrouter');
...
StarRouter.prototype.handle = function (stanza) {
    ...
    if (!stanza.attrs.to) {
        this.logger.warn('discarded message because to attribute is missing: ' + stanza.toString());
        return;
    }
    ...
    for (var i = 0, l = this.routes.length; i < l; i++) {
        ...
        if (!handled) {
            try {
                this.logger.debug('check if message fits to: ' + route.route.name);
                // handle events
                if (route.route.match(stanza)) {
                    this.logger.debug('handle stanza with ' + route.route.name);
                    if (route.type == 'reciever') {
                        this.logger.debug('use handle');
                        // call handle for Reciever
                        handled = route.route.handle(stanza);
                    } else {
                        // call send for Sender
                        this.logger.debug('use send');
    ...
    if (!handled) {
        this.logger.warn('discarded message: ' + stanza.toString());
    }
};
...
StarRouter.prototype.listenTo = function (route) {
    ...
    // store routes
    this.routes.push({route: route, type: 'sender'});
...
StarRouter.prototype.chain = function (route) {
    ...
    // store routes
    this.routes.push({route: route, type: 'reciever'});
