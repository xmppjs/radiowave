...
OAUTH2.prototype.authenticate = function (opts) {
  ...
  return new Promise(function (resolve, reject) {
    logger.info('OAUTH2 authenticate ', opts.oauth_token); // jshint ignore:line
    self.verifyToken(opts.username, opts.oauth_token, function (err, user) { // jshint ignore:line
      // error
      if (err) {
      ...
      // everything is okay
        ...
        logger.debug('OAUTH2: token ' + opts.oauth_token + ' is valid ' + JSON.stringify(user)); // jshint ignore:line
        resolve(user);
      }
    });
  });
